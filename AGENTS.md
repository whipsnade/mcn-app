...
- `app/core/config.py` 在启动时做硬性校验：`TENCENT_PLAN_BASE_URL` 必须为已确认的腾讯端点、`MCP_CALL_POINTS` 必须为 10、密钥不得为空。
