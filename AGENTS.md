...
backend/            FastAPI 后端
  app/
    ...
    reporting/      收藏（只读）与版本化分析报告；analysis_reports.py 会话级（task_id 可空）
