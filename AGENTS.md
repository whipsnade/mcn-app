...
backend/            FastAPI 后端
  app/
    ...
    quick/          快捷功能（行业标签解析 tags.py、同步计费调用 service、/quick/* 端点：达人推荐/达人详情/爆贴/评估上传）
...
- `app/core/config.py` 在启动时做硬性校验：`MCP_CALL_POINTS` 必须为 10、密钥不得为空。模型供应商可自由配置：`TENCENT_PLAN_BASE_URL` / `TENCENT_PLAN_MODEL` / `TENCENT_PLAN_API_KEY` 支持任意 OpenAI 兼容端点（腾讯 Token Plan、月之暗面 Kimi 等）。
