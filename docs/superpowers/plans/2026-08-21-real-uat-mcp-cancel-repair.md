...
- [x] candidate-r7（`35ccc37`）对应远程 Actions run `32473243258`：Migration safety、Frontend、Pi Runtime、Pi Gateway 全绿；Browser E2E 因 CI 未提供 Playwright 配置默认的 `backend/.venv/bin/python` 失败，Backend 仍只有步骤级 Pytest exit code 1。
- [ ] 以 `e61b473` 重建 candidate-r8，并取得远程 CI 全绿；在此之前不得部署预发布或执行真实 Web UAT：通过门槛后仅执行一次专用 UAT 租户的“Direct MCP + 取消”组合 Web UAT：一次瑞幸咖啡请求，Attempt=1、模型≤10、DataTap≤5、积分≤50、retries=0、fresh Run=0。
