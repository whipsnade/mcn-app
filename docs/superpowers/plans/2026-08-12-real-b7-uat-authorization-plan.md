...
| Pi 内部工具 | 6 个（Direct MCP 架构现行：`get_session_context`/`load_marketing_skill`/`read_artifact`/`build_artifact_draft`/`publish_artifacts`/`request_clarification`；历史 13 个含 search_evidence/read_tool_result 与 6 个旧 Builder 的清单已废止，见 §4.2） | `backend/app/pi_gateway/service.py` claim 响应 |
...
> 本节为纠偏后的现行规则声明；本文档其余历史章节保留原样，仅在冲突处按本节优先。

1. 新 Pi production path 不产生数据库 `Evidence` 业务实体、不使用 `mcp_result_v1`、
   不做 available/empty/unavailable 业务分类、不要求 `search_evidence`/`read_tool_result`、
   不要求模型先写数据库 Evidence 再调用 Builder。标准 MCP Tool Result 由 adapter 原样
   交给 Pi/模型；accounting finalize 只传 metadata；Builder（`build_artifact_draft`）
   接收模型自主选择的 `artifact_type + payload`，只执行 RuntimeSnapshot allowlist、严格
   payload Schema、tenant/session/run、Publication、Version 与 lineage 结构校验。
2. CompletionValidator 不以数据库 Evidence 数量或 required artifact 作为 Runtime 完成
   条件；无 Artifact 的文字 Run 可以合法 completed。
3. 文档中需要表达测试证明时使用：验收证据、数据库快照、事件记录、账本对账、ToolCall
   记录——不再统称为数据库 `Evidence`。旧 Evidence 表、历史数据与 current/legacy 路径
   保留兼容，不删除、不改写。
4. 本文档中的「必需 Evidence」「Evidence lineage」「tool_call Evidence」等表述，在 L2
   场景部分已按上述语义纠偏（§6.3）；其余历史章节中出现时按历史设计理解。
...
| migration head | `0044_agent_run_loop_guard`（唯一 head；迁移已完成，只核验不重建；`0043_billing_downgrade_guard` 为第一轮 B7 时点的历史事实） | `alembic heads` |
...
### 4.2 Pi 内部工具（2026-08-13 Direct MCP 架构纠偏后现行 6 个）

> 历史清单（13 个，含 `search_evidence`/`read_tool_result` 与 6 个旧 Builder）已被 Direct
> MCP 架构废止：新 Pi production path 不要求模型先写数据库 Evidence 再调用 Builder，模型
> 通过统一 `build_artifact_draft` 提交 allowlist 内产物。历史清单仅作为历史事实保留于本节。
...
`publish_artifacts`、`request_clarification`。

历史清单（已废止，仅历史事实）：`search_evidence`、`read_tool_result`、
`build_brand_report_draft`、`build_campaign_report_draft`、`build_kol_selection_draft`、
`build_kol_analysis_draft`、`build_kol_detail_draft`、`build_insight_draft`。
...
| L0-02 | migration head 核对 | `alembic heads` 唯一且为 `0044_agent_run_loop_guard` | NEEDS_USER_APPROVAL |
...
**L2-01 品牌报告完整链路（2026-08-13 Direct MCP 架构纠偏后）**
真实模型 是｜真实 DataTap 是｜测试钱包变更 是｜状态变更 否
输入形态：用户通过 Web 明确要求完成品牌分析并生成正式品牌报告（自然语言业务目标，
不写工具名/artifact_type/Schema/Builder 名称或调用次数，见 §8 输入示例）。
执行形态：Pi/模型自主选择 MCP 工具、调用顺序与调用次数；Pi/模型自主调用 allowlist 中的
Artifact Skill（`build_artifact_draft`）。不使用服务端 required artifact contract；不要求
数据库 Evidence 或 search_evidence/read_tool_result 必经步骤。
「必须产生品牌报告」只是本 UAT 场景的 **PASS 结果断言**，不是 Runtime 完成门禁：
Run 只完成文字回答而无正式报告时，Runtime 可以合法 completed，但本场景判 FAIL。
PASS 要求：
- 单次 Web 提交；单个 Run、Attempt 1；Run completed；无 running/unknown ToolCall；
  reserved=0；
- 当前 Run 生成 allowlist 内品牌报告 Draft；Publication 与 Version 成功；
  Excel 导出与 BI 读取绑定同一 Version；lineage 归属当前 tenant/session/run；
- 数据库 Evidence 增量为 0（新 Pi 路径预期事实）；
- 不出现 mcp_result_v1、result_unavailable 业务分类或 Evidence Bridge 回灌。
验收证据（QA 证明，非数据库 Evidence）：账本对账、事件记录、ToolCall 记录、Version/
导出 SHA-256、数据库快照。
场景特有停止条件：Artifact 无法通过发布门禁即停
回滚动作：无（只读类业务产物保留作验收证据）｜授权级别 L2

**L2-02 活动报告完整链路（2026-08-13 Direct MCP 架构纠偏后）**
真实模型 是｜真实 DataTap 是｜测试钱包变更 是｜状态变更 否
与 L2-01 同样原则：场景目标是验收断言，不是 Runtime required contract；不要求数据库
Evidence；Pi/模型自主选择 MCP 工具与调用次数，Artifact 必须来自 Snapshot allowlist
（`campaign_report_v2` 或 `campaign_report_v3`）并过严格 Schema。
PASS 要求：单次 Web 提交；单 Run、Attempt 1、completed；reserved=0；当前 Run 生成
allowlist 内活动报告 Draft，Publication/Version 成功，Excel 与 BI 绑定同一 Version；
数据库 Evidence 增量 0；无 mcp_result_v1/Evidence Bridge 回灌。
场景特有停止条件：Artifact 无法通过发布门禁即停
回滚动作：无｜授权级别 L2

**L2-03 KOL 圈选、KOL 分析与 Excel 导出（2026-08-13 Direct MCP 架构纠偏后）**
真实模型 是｜真实 DataTap 是｜测试钱包变更 是｜状态变更 否
与 L2-01 同样原则：场景目标是验收断言；不要求数据库 Evidence 或评分快照 Evidence；
Artifact 必须来自 Snapshot allowlist（`kol_selection_v3`/`kol_analysis_v2`）并过严格
Schema。圈选名单与导出内容一致性由导出时 Version 绑定校验。
PASS 要求：单次 Web 提交；单 Run、Attempt 1、completed；reserved=0；allowlist 内
圈选/分析 Artifact 发布成功；Excel 与 BI 绑定同一 Version；数据库 Evidence 增量 0。
场景特有停止条件：圈选名单与导出内容不一致即停
回滚动作：无｜授权级别 L2

**L2-04 钻取绑定确切 Version，0 新 DataTap（2026-08-13 Direct MCP 架构纠偏后）**
真实模型 是｜真实 DataTap **否（0 次新外发为硬断言）**｜测试钱包变更 否｜状态变更 否
预期 Artifact：`insight` 模块 Version，parent 绑定确切父 Version（同 Session 已发布）；
预期 terminal `completed`。不要求 Evidence lineage：新 Pi 路径的钻取校验是
`build_artifact_draft` 对父 Artifact/Version 同 Session 已发布的归属校验 + Publication
的 Version lineage 结构校验，而非数据库 Evidence 引用。
验收证据：父 Version 读取记录、lineage parent 关联、0 外发证明（无外发日志/无新 ToolCall）
场景特有停止条件：发生任何新 DataTap 外发即停
回滚动作：无｜授权级别 L2

**L2-05 澄清输入：0 Artifact、0 MCP、0 扣费（2026-08-13 Direct MCP 架构纠偏后）**
真实模型 是｜真实 DataTap 否｜测试钱包变更 **否（0 扣费为硬断言）**｜状态变更 否
预期 Artifact：无；预期 terminal `clarification_requested`。澄清/拒答可以合法地只有文字
结果：Runtime 不要求任何 Artifact 或数据库 Evidence 作为完成条件。
验收证据（QA 证明，非数据库 Evidence）：0 ToolCall、0 账本流水、0 Artifact 的数据库快照
场景特有停止条件：发生扣费/外发/Artifact 即停
回滚动作：无｜授权级别 L2

**L2-06 非营销拒答：0 Artifact、0 MCP、0 扣费（2026-08-13 Direct MCP 架构纠偏后）**
真实模型 是｜真实 DataTap 否｜测试钱包变更 否（硬断言）｜状态变更 否
预期 Artifact：无；预期 terminal `completed`（拒答回答）。纯文字拒答是合法的 Runtime
完成形态，不要求 Artifact/数据库 Evidence。
验收证据：同 L2-05（数据库快照证明 0 ToolCall/0 流水/0 Artifact）
场景特有停止条件：同 L2-05
回滚动作：无｜授权级别 L2
...
验收证据（QA 证明，非数据库 Evidence）：0 外发证明、钱包 0 变动、无 reserve/settle/release 流水
场景特有停止条件：发生任何真实外发即停
...
验收证据（QA 证明，非数据库 Evidence）：暂停前 settled 记录、暂停后 preflight 拒绝记录、0 新增外发/流水证明
...
验收证据（QA 证明，非数据库 Evidence）：跨租户读 session/run/events/Artifact/账务全部 404 的证明、DB 层互不可见抽查
...
验收证据（QA 证明，非数据库 Evidence）：事件序号单调递增、`message.completed` 先于唯一终态、断线 `Last-Event-ID` 重连无洞无重复
...
验收证据（QA 证明，非数据库 Evidence）：cancel ack、未外发预留释放记录、终态唯一性证明
...
验收证据（QA 证明，非数据库 Evidence）：Attempt 1 丢失 → 恢复恰好一次新 Attempt → 再次丢失 → 稳定 failed；崩溃窗口内 ToolCall 置 unknown 且 0 重放
...
验收证据（QA 证明，非数据库 Evidence）：draining 期间新 Run 0 claim、在途完成、恢复 active 后 queued 被领取
...
验收证据（QA 证明，非数据库 Evidence）：切换只影响新 Run、同一消息不被两个 Runtime 执行、旧 Run snapshot 不变
...
验收证据（QA 证明，非数据库 Evidence）：置位后新 Run `runtime_backend=current`、在途 Pi 不被杀、历史 snapshot 不变
...
验收证据（QA 证明，非数据库 Evidence）：激活新版本后旧 Run `runtime_config_snapshot_json` 逐字节不变、版本指针不变
场景特有停止条件：任何历史 snapshot 变化即停
回滚动作：无（append-only，旧版本自然 retired）｜授权级别 L2+D(Runtime Config 激活)

**L2-17 Artifact / Version / Excel / BI / B0 Gate 同版本（2026-08-13 改名并纠偏）**
真实模型 否（复用 L2-01/02/03 产物做核验）｜真实 DataTap 否｜测试钱包变更 否｜状态变更 否
预期 Artifact：无新产物（核验性场景）；预期 terminal NOT_APPLICABLE
验收证据：Excel 导出与 BI 详情绑定同一 Version、B0 发布门禁对正式产物复核零 issue、
Version 结构 lineage 一致。**不再要求数据库 Evidence 同版**（新 Pi 路径不产生数据库
Evidence；旧名称中的 "Evidence" 已删除）。
...
验收证据（QA 证明，非数据库 Evidence）：账本逐笔清单、ToolCall 状态分布、净支出恒等式计算过程
...
真实模型 否｜真实 DataTap 否｜测试钱包变更 否（只读核对）｜状态变更 否
预期 Artifact：无；预期 terminal NOT_APPLICABLE
验收证据（QA 证明，非数据库 Evidence）：`RuntimeUsageRecord` 与模型请求逐条对应、无重复计费、usage 未进用户 SSE/prompt
...
预期 Artifact：无；预期 terminal NOT_APPLICABLE
验收证据（QA 证明，非数据库 Evidence）：无残留进程/端口/租约/nonce 证明、测试数据保留/清理清单
...
3. 启动门禁（fail-closed，任一不符即 B7_BLOCKED）：工作树干净、HEAD 为 audited Direct
   ...
   （值仅进程内）。2026-08-12 首次尝试即因工作树未提交改动在此 fail-closed（历史事实，
   当时的门禁基线为 `68deca58…`）。
...
- 自然语言测试输入只表达业务目标，不写工具名、artifact_type、Schema、Builder 名称或
  调用次数。示例：

  > 「请基于真实数据完成蔚来品牌分析，生成正式品牌分析报告，并确保可以在 BI 中查看和
  > 导出 Excel。请自行决定需要查询的数据和分析步骤。」

- Pi/模型自主选择 MCP 工具、调用顺序与调用次数；Artifact 发布只允许当前 Snapshot
  allowlist。不使用服务端 required artifact 强迫 Builder。
- 禁止：在 prompt 中规定具体 MCP 工具/顺序/次数；注入预计算 MCP 结果；使用 fake model；
  在真实调用发生后创建 fresh Run 重试；将旧 Run/Artifact/Version 用作当前场景结果。

### 8.3 预算（防失控紧急上限，不是工具规划策略）
...
| Web 用户提交 | 严格 1 次 |
| 业务 Run | 严格 1 个 |
| `limits.max_decisions` | 60 |
| 真实模型请求 | 最多 60 次 |
| DataTap discovery | 最多 5 次 |
| DataTap 业务 dispatch | 最多 50 次 |
| 钱包净支出 | 最多 500 积分 |
| Artifact 发布 | 只允许当前 Snapshot allowlist |
| Gateway event buffer | 使用默认有界 buffer，不通过提高 buffer 掩盖问题 |

这些是防止失控的紧急上限；不得因为达到常见调用数量就提前干预模型，不得把上限当作
工具规划策略或 prompt 指令。

### 8.4 启动规则

- 发生真实模型或业务 DataTap 外发前，允许修正环境路径并重启服务（不再设置任意的
  FastAPI/Gateway 启动次数上限）；
- 一旦业务 Run 已发生真实外部调用，不得通过重启和 fresh Run 隐藏失败；
- 环境问题（启动路径、配置、隔离）与业务失败（模型输出、Artifact 校验）必须分别报告。
