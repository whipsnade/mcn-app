...
测试通过依赖注入内存审核 manifest。真实环境在 Task 14 的单独授权步骤中只执行五服务 `initialize/tools/list`，把发现项写入 quarantine；人工核对名称、描述和 Schema 后再生成新的受版本控制 manifest。未进入 manifest 的工具即使上游存在也不能被 Planner 看见或调用。
...
- [ ] **Step 2: 运行测试并确认现有钱包缺少批次和终态互斥**

Run: `cd backend && .venv/bin/pytest tests/billing/test_wallet_batch.py tests/billing/test_wallet_concurrency.py tests/mcp_gateway/test_billing_lifecycle.py -q`

Expected: `reserve_batch` 不存在或终态测试失败。
...
- [ ] **Step 2: 运行测试并确认 orchestration 模块不存在**
...
Run: `cd backend && .venv/bin/pytest tests/orchestration tests/model tests/mcp_gateway/test_registry.py -q`
...
- [ ] **Step 4: 验证恢复、取消、租约和旧 API**

Run: `cd backend && .venv/bin/pytest tests/tasks tests/orchestration tests/mcp_gateway tests/billing -q`
...
- [ ] **Step 2: 运行测试并确认 reporting 服务缺少实现**

Run: `cd backend && .venv/bin/pytest tests/reporting -q`

Expected: normalizer/scoring 导入失败。
...
- [ ] **Step 4: 验证 API、版本一致和完整后端闭环**

Run: `cd backend && .venv/bin/pytest tests/reporting tests/workspace tests/tasks/test_executor.py -q`

Expected: 排序/筛选、报告候选版本、收藏幂等、跨用户 404、会话历史恢复和最终事件顺序全部通过。
...
- [ ] **Step 2: 运行测试并确认 reducer 和 hook 不存在**
...
- [ ] **Step 4: 验证类型、事件和现有认证恢复**

Run: `npm run test -- src/state/taskEvents.test.ts src/hooks/useTaskStream.test.tsx src/hooks/useWorkspace.test.tsx && npm run lint`

Expected: 任务事件不重复；重连携带最后 ID；abort 不调用取消；旧用户/旧会话事件被忽略；TypeScript 零错误。
...
Run: `npm run test -- src/components/WorkspaceTabs.test.tsx src/components/CandidateList.test.tsx src/components/FavoritesPanel.test.tsx src/components/ChatArea.test.tsx`
...
- [ ] **Step 4: 验证组件交互和完整前端回归**

Run: `npm run test && npm run lint && npm run build`

Expected: 所有组件测试通过、类型检查零错误、Vite 构建成功；现有按钮、登录、会话和移动导航测试不回归。
...
- [ ] **Step 2: 运行测试并确认旧 BI 类型不匹配**

Run: `npm run test -- src/components/BiReport.test.tsx`

Expected: 旧 `ReportData` 属性或新版 props 不存在导致失败。
...
- [ ] **Step 4: 验证 BI 组件、类型和桌面/移动行为**

Run: `npm run test -- src/components/BiReport.test.tsx && npm run lint && npm run build`

Expected: 九段报告、版本门控、空数据、图表测试通过；构建成功。
...
| 安全、脱敏、指标、10 并发、崩溃窗口和视觉回归 | Task 5、6、8、14 |
| fake-only 测试、真实冒烟单独授权和回滚手册 | Task 14 |
...
- Task 14 全部门槛通过，无真实供应商请求。
- 10 并发无透支、重复扣费、冻结积分、死锁残留和跨用户数据泄露。
- 崩溃窗口、SSE 断线、安全扫描和视觉回归通过。
- 用户单独批准真实冒烟后，才配置真实凭据并执行最多一次成功调用；冒烟结果和 10 积分消费单独记录。
