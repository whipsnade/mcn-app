...
describe('SkillAdmin', () => {
  ...
  it('loads revisions, shows active/audit fields, edits content, and creates a revision', async () => {
    ...
    expect(await screen.findByText('生产 / 全量 · v2')).toBeTruthy();
  ...
  it('shows structured validation errors and loads database diff', async () => {
    ...
    expect(mockDiff).toHaveBeenCalledWith('brand-research', 1, 2);
  });
  ...
  it('activates with tenant rollout or full rollout and confirms rollback in dialog', async () => {
    ...
    await waitFor(() => {
      expect(mockActivate).toHaveBeenCalledWith(
        'brand-research',
        { revision: 2, tenant_id: 'tenant-1', environment: 'production', rollout_percent: 25 },
        expect.any(String),
      );
    });
    ...
    await waitFor(() => {
      expect(mockActivate).toHaveBeenCalledWith(
        'brand-research',
        { revision: 2, tenant_id: 'tenant-1', environment: 'production', rollout_percent: 100 },
        expect.any(String),
      );
    });
