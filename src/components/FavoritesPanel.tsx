...
export default function FavoritesPanel({ refreshKey, onCountChange, onFavoritesChange }: FavoritesPanelProps) {
  ...
  if (loading) return <div className="flex flex-1 items-center justify-center bg-slate-50 text-xs font-medium text-slate-400">正在加载收藏…</div>;
  if (!favorites.length) return <div className="flex flex-1 items-center justify-center bg-slate-50 text-xs font-medium text-slate-400">还没有收藏的达人</div>;

  return <div className="min-h-0 flex-1 overflow-y-auto bg-slate-50 p-4"><div className="space-y-2">{favorites.map(favorite => <div key={favorite.kol_id} className="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm"><div><div className="text-xs font-semibold text-slate-800">{favorite.kol_id}</div><div className="mt-1 text-[10px] text-slate-400">{favorite.platform} · 跨会话收藏</div></div><button type="button" aria-label={`取消收藏 ${favorite.kol_id}`} onClick={() => void remove(favorite.kol_id)} className="rounded-lg p-1.5 text-amber-500 transition hover:bg-amber-50"><Star className="h-3.5 w-3.5 fill-amber-400" /></button></div>)}</div></div>;
}
