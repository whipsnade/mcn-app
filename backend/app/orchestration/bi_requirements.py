...
class MetricDef:
    ...
    ``content_signal`` 为可选的内容特征兜底：source_tools 之外的
    insight 统计工具调用结果若带有该特征（如按天日期序列），同样
    视为覆盖——模型有权用等价查询拿到同一数据项。
    """
...
BI_REQUIRED_METRICS: tuple[MetricDef, ...] = (
    ...
    MetricDef(
        ...
        # query.analysis 等统计工具按天拆分时可拿到等价数据（真实案例：
        # trend 上游连续超时，模型改用 query.analysis 查到了逐日声量/曝光）。
        content_signal="daily_series",
    ),
    MetricDef(
...
# 内容特征兜底：只认 insight 统计/查询家族工具的结果，避免 KOL 搜索结果里
# 恰好出现的发布日期被误判为"按天走势"。
_INSIGHT_TOOL_PREFIX = "datatap.insight."
_DATE_RE = re.compile(r"\d{4}-\d{2}-\d{2}")
_DAILY_SERIES_MIN_POINTS = 3
...
def _matches_daily_series(summary: Any) -> bool:
    """按天序列特征：摘要中出现至少 N 个不同的日期（YYYY-MM-DD）。"""
    return len(set(_DATE_RE.findall(_summary_text(summary)))) >= _DAILY_SERIES_MIN_POINTS
...
def metric_coverage(
    ...
    checklist = tuple(BI_REQUIRED_METRICS) if metrics is None else tuple(metrics)
    covered: set[str] = set()
    ...
    # 内容特征兜底：source_tools 之外的 insight 工具调用若带特征信号
    # （如按天日期序列），同样计入覆盖。
    signal_metrics = [
        metric
        for metric in checklist
        if metric.content_signal == "daily_series" and metric.key not in covered
    ]
    if signal_metrics:
        for note in trajectory.results:
            if note.status != "settled" or _is_empty_summary(note.summary):
                continue
            if not note.tool.startswith(_INSIGHT_TOOL_PREFIX):
                continue
            if _matches_daily_series(note.summary):
                covered.update(metric.key for metric in signal_metrics)
                break
    ...
    return tuple(
        {
            ...
            "content_signal": metric.content_signal,
        }
