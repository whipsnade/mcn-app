...
async def export_latest_task_xlsx(
    ...
    filename = _safe_filename(f"{session.brand}_{session.category}_KOL匹配度分析_{stamp}.xlsx")
...
def _export_candidate(item: TaskCandidatePoolItem, kol: Kol, snapshot: KolSnapshot) -> ExportCandidate:
    normalized = snapshot.normalized_json or {}
    dimensions = item.score_breakdown_json.get("dimensions", {})
    scores = {
    ...
    raw_values = normalized.get("export_fields", {})
    values = dict(raw_values) if isinstance(raw_values, dict) else {}
    values.setdefault("engagement_rate", _percentage_text(normalized.get("engagement_rate")))
    ...
    return ExportCandidate(
        ...
        values=values,
        score_reason="数据缺失字段按评分规则处理" if normalized.get("missing_fields") else "基于规范化 MCP 数据评分",
...
__all__ = ["CONTENT_TYPE", "ExportCandidate", "render_workbook"]
